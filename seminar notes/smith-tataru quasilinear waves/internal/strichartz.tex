...
\begin{figure}[h]
        ...
        \caption{On time scale $|t_1 - t_2| > 2\lambda^{-1}$ and close to the cone $|\delta u| \leq 4 \lambda^{-1}$, it is easy to rotate a tube containing the point to obtain another tube containing the point.  }\label{fig:closecone}
    \end{figure}
...
In this case, the wave packets have had some time to decohere. If $\mathtt P_2$ is in the interior, i.e. $\delta u < -4 \lambda^{-1}$, then there can be no slabs containing both points by Huygen's principle, see Figure \ref{fig:interesting},
    ...
    \]
We now turn to the interesting case $\delta u \geq 4 \lambda^{-1}$, where $\mathtt P_2$ is in the exterior of the cone. 
...
\begin{figure}[h]
        ...
        \caption{On time scale $\lambda^{-1}$, the wave packets are still approximately maximally-overlapping, so we use the trivial bound. }\label{fig:interesting}
    \end{figure}

...
\end{figure}
    ...
    \begin{proof}
        Suppose $\| \partial \bfg(t) \|_{C^{0, 0+}_x} \geq \lambda$, then the bound follows from Cauchy-Schwartz in $\theta$. Indeed, since there are $O((\frac{\lambda}{\epsilon_0})^{\frac{n - 1}{2}})$-many angles, 
            ...
            \end{align*}
        then integrating and applying Bernstein gives the result. 
    ...
    \begin{remark}
        Note that we used $\tfrac{n - 1}{2} \leq 2$ for dimensions $n \leq 5$ to estimate the bad-time contribution. 
    \end{remark}

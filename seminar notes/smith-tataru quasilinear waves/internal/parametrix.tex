...
\begin{remark}
    Note that we used the Sobolev embedding for $s - 1 > \tfrac{n - 1}{2}$. 
\end{remark}
...
\begin{remark}
    It is instructive to compare the computation above against the flat case. When $u$ is optical, 
        \[
            \Box \delta(u) 
...
Evidently these hold in the flat case, we want to show these persist for the variable background.     
        ...
        It is easy to check 
            \[
                \| \Phi_\theta \|_{L^2_{y_\theta} H^{\frac{n - 1}{2} + }_{y'_\theta} (\R^n)}^2 
                    \lesssim (\epsilon_0)^{\frac{n - 1}{2}} \sum_j \| \psi^{\theta, j} \|_{H^{\frac{n - 1}{2} +} (\R^{n - 1})}^2.
            \]
        after rescaling. 
    ...
    \begin{proof}[Proof of Proposition \ref{prop:parametrix} (\ref{item:WPenergy})]
    ...
    \end{proof}
     ...
     \begin{proof}[Proof of Proposition \ref{prop:parametrix} (\ref{item:WPerror})]

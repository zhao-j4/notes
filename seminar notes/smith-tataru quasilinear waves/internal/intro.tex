...
Following the standards set by Hadamard and Kato, we say that the initial data problem for the quasi-linear wave equation \eqref{QNLW} is \textit{locally well-posed} in $(H^s_x \times H^{s - 1}_x) (\R^n)$ if the following hold: 
	\begin{enumerate}
...
Scaling 
	...
	\]
This leads to the scaling critical Sobolev exponent $s_{\text{crit}} := \tfrac{d}{2}$. The main result: half a derivative above scaling. 
...
\begin{theorem}[Smith-Tataru \cite{SmithTataru2005}]
...
\begin{remark}
	The proof breaks down in higher dimensions $n \geq 6$ due to a technical failure in the orthogonality argument for the wave packet decomposition. 
\end{remark}
...
\begin{theorem}[Energy estimate \cite{HughesEtAl1977}]
